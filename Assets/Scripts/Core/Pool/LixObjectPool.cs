...
namespace Lix.Core
{
  public abstract class LixObjectPool<T> : IObjectPool<T> where T : class
  {
    ...
    public LixObjectPool(int defaultCapacity, int maxSize, bool prewarm = true, bool collectionCheck = true)
    {
      ...
      if (prewarm)
      {
        ...
        Debug.Log("Created LixObjectPool pool for " + typeof(T) + " with " + Pool.CountAll + " objects");
      }
    }
